// This code has been copied from "github.com/mccoyst/ogg/encode.go" or
// https://github.com/mccoyst/ogg/blob/master/encode.go and altered.
...
func (w *Encoder) writePage(page []byte, h *pageHeader) error {
	...
	bb := hb.Bytes()
	crc := crc32(bb)
	_ = binary.Write(bytes.NewBuffer(), byteOrder, crc)

	//data := []byte{}
	//for i:=0; i<len(hb); i++ {
	//	data[i], _ = hb.ReadByte()
	//}

	_, err := w.writer.Write(bb)
	//data := []byte{hb}
	//err := ioutil.WriteFile(w.writer, hb, 0644)
	//_, err := hb.WriteTo(w.writer)
	return err
}